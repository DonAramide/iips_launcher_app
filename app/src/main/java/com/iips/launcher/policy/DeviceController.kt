...
object DeviceController {
    ...
    private fun isKioskSecurityReady(context: Context): Boolean {
        ...
        if (!ready) {
            android.util.Log.i(
                "DeviceController",
                "${KioskController.GATE_MARKER} Kiosk/security skipped: " +
                    "DeviceOwner=${DeviceAdminReceiver.isDeviceOwner(context)} " +
    ...
    fun enableComprehensiveSecurity(context: Context) {
        try {
            ...
            // Never apply debugging/USB restrictions from Device Owner alone.
            // ADB stays available until PolicyEnforcementEngine applies a valid enrolled policy.
